...
import { WorkspaceValidator } from './validation/workspaceValidator.js';
import { FileSystemSessionStorage } from './storage/SessionStorage.js';
import { ConversationHistory } from './history/ConversationHistory.js';
...
class SessionManager {
  ...
  async cleanupExpiredSessions() {
    ...
    const cleanupPromises = expiredSessions.map(sessionId => 
      this.removeSession(sessionId, { removeFromStorage: false })
        .catch(err => {
          console.error(`[SessionManager] Error removing session ${sessionId}:`, err);
          return false;
        })
    );
    ...
    // Log results
    if (expiredSessions.length > 0 || removedCount > 0) {
      console.error(
        `[SessionManager] Cleaned up ${expiredSessions.length} in-memory ` +
        `and ${removedCount} stored expired sessions`
      );
    }
    
    return {
      inMemory: expiredSessions.length,
      storage: removedCount || 0
    };
  }
